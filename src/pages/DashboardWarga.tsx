...
export default function DashboardWarga({ warga }: Props) {
  ...
  const cards = [
    { icon: '📨', label: 'Surat Saya', value: stats.totalSurat, color: 'from-blue-500 to-blue-600' },
    { icon: '⏳', label: 'Diproses', value: stats.menunggu, color: 'from-yellow-500 to-orange-500' },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-2xl p-6 text-white shadow-lg">
        <h1 className="text-2xl font-bold">Halo, {warga.nama_lengkap}! 👋</h1>
        <p className="text-indigo-100 mt-1">NIK: {warga.nik} • RT {warga.rt} / RW {warga.rw}</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100">
        <div className="p-4 border-b border-gray-100">
          <h2 className="font-bold text-gray-800">📜 Surat Terbaru Saya</h2>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {recentSurat.length === 0 ? (
                <tr><td colSpan={5} className="text-center py-8 text-gray-400">Belum ada surat</td></tr>
              ) : recentSurat.map((s) => (
